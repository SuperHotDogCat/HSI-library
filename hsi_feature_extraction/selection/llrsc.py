...
class LaplacianregularizedLowRankSubspaceClustering(BaseFeatureExtractor):
    ...
    def __init__(
        ...
        Args:
            num_bands_to_select(int or None): The Number of bands to select. If "None", it will be Automatically determined.
                                              Using “num_bands_to_select=None” may cause an error.It is recommended to set any value first.
    ...
    def get_num_channels(self) -> int:
        if not hasattr(self, "num_bands_to_select"):
            raise AttributeError("num_bands_to_select does not exist.")
    ...
    def fit(self, X: np.array) -> None:
        ...
        if not self.num_bands_to_select:
            self.num_bands_to_select = int(
                np.mean(self.estimate_band_subset_size())
            )
        self.selected_bands = self.select_bands(self.num_bands_to_select)
        self.select_most_frequent(self.selected_bands)
    ...
    def select_bands(self, n_select: int) -> np.array:
        ...
        for i in range(self.batch_size):
            clustering = SpectralClustering(
                n_clusters=n_select, affinity="precomputed"
            )
            ...
            for cluster in range(n_select):
                ...
                closest_band = cluster_indices[
                    np.argmin(
                        np.sum(
                            (self.X[i, cluster_indices, :] - centroid) ** 2,
                            axis=1,
                        )
                    )
                ]
    ...
    def select_most_frequent(self, matrix):
        ...
        self.selected_bands = [i[0] for i in self.selected_bands_detail]
